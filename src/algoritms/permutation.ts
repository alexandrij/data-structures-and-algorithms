...
export function swap<T = any>(arr: T[], i: number, j: number): T[] {
    const temp = arr[i];
    arr[i] = arr[j];
    arr[j] = temp;
    return arr;
...
export function permutation(input: string | string[], start: number = 0) {
    ...
    for (let i = start; i < arr.length; i++) {
        ...
        if (i < arr.length - 1) { swap<string>(arr, start, i + 1); }
    }
